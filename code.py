...
class Property:
    ...
    def createBooking(self):
        ...
        self.saveBooking(guest, check_in_date, check_out_date, total_fee)
        self.createInvoice(guest, check_in_date, check_out_date, total_fee)

    def saveBooking(self, Pguest, Pcheck_in_date, Pcheck_out_date, Pfee):
        clashed = False
        cursor.execute(
        ...
        if clashed:
            ...
            if Pcheck_out_date is None:
                print("Booking cancelled")
                return
            self.saveBooking(Pguest, Pcheck_in_date, Pcheck_out_date, Pfee)
        else:
            cursor.execute(
                "INSERT INTO bookings (property_id, guest, check_in, check_out, fee) VALUES (?, ?, ?, ?, ?)",
                (self.id, Pguest, Pcheck_in_date.isoformat(), Pcheck_out_date.isoformat(), Pfee)
            )
            conn.commit()
    ...
    def showBookings(self, ask_sort=True):
        if ask_sort:
            invalid = True
            while invalid:
            ...
            display_list = self.sortBookings(sortby, reverse)
        else:
            cursor.execute(
    ...
    def createInvoice(self, Pguest, Pcheck_in_date, Pcheck_out_date, Pfee):
